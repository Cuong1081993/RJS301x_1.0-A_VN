...
const Cart = (props) => {
  ...
  const submitOderHandler = async (userData) => {
    ...
    const response = await fetch(
      "https://meals-3e083-default-rtdb.firebaseio.com/orders.json",
      {
        method: "POST",
        body: JSON.stringify({
          user: userData,
  ...
  const modalActions = (
    <div className={classes.actions}>
      ...
      {hasItems && (
        <button className={classes.button} onClick={oderHandler}>
          Order
        </button>
      )}
    </div>
  ...
  const cartModalContent = (
    <Fragment>
      ...
      {isCheckOut && (
        <Checkout onConfirm={submitOderHandler} onCancel={props.onClose} />
      )}
